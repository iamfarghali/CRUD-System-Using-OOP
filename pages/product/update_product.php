...
<form action="<?= htmlspecialchars($_SERVER['PHP_SELF']).'?id='.$product->id ?>" method="post" accept-charset="utf-8">
	            	...
	            	<select class="form-control" name="category_id">
	            		...
	            		<?php
	            			while( $row = $categoryStmt->fetch(PDO::FETCH_ASSOC)) {
		            			extract($row);
	            		?>
		            			<option value="<?=$id?>" <?php if ($id == $product->category_id) echo 'selected'; ?>>  <?=$name?> </option>
	            		<?php
	            			}
	            		?>
	            	</select>
